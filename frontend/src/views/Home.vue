  ...
  try {
    ...
    if (response.success && response.data !== null && response.data !== undefined) {
      ...
      try {
        ...
        // 短暂延迟后跳转
        setTimeout(async () => {
          console.log('[Debug] 开始导航到 /result')
          try {
            await router.push('/result')
            console.log('[Debug] 导航成功')
          } catch (navErr: any) {
            console.error('[Debug] 导航失败:', navErr)
          }
        }, 500)
      } catch (e: any) {
        console.error('保存旅行计划失败:', e)
        message.error('旅行计划数据过大，保存失败: ' + (e.message || ''))
      }
    } else {
      console.log('[Debug] 响应success为false或无data:', response)
